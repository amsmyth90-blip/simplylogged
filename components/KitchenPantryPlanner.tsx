...
import { PantryHeader } from "@/components/kitchen-pantry/PantryHeader";
import { mergeKitchenItems, normaliseKitchenItem } from "@/components/kitchen-pantry/pantry-items";
import { selectPantryPhotos } from "@/components/kitchen-pantry/pantry-photo-selection";
...
export function KitchenPantryPlanner() {
  ...
  const addPhotos = (files: File[]) => {
    setSelectedFiles((current) => {
      const selection = selectPantryPhotos(current, files);
      setErrorMessage(selection.error);
  ...
  const analysePhotos = async () => {
    ...
    try {
      ...
      setConfirmedIngredients(new Set(payload.analysis.ingredients.map(item => normaliseKitchenItem(item.name))));
      setSelectedMealIndex(0);
      setStage("confirm");
    } catch (error) {
  ...
  const confirmStock = () => {
    ...
    const names = analysis.ingredients
      .filter(item => confirmedIngredients.has(normaliseKitchenItem(item.name)))
      .map(item => item.name);
    updateState(current => ({
      ...current,
                ...
                <div className="mt-2 flex gap-2 overflow-hidden">
                  ...
                  {selectedFiles.length < 8 ? (
                    <button type="button" onClick={() => cameraInputRef.current?.click()}
                      className="flex h-14 w-14 items-center justify-center rounded-xl border border-dashed border-[#91a889] bg-[#f2f6ef] text-[#64805b]"
                      aria-label="Add another photo"><UiIcon name="plus" className="h-4 w-4" /></button>
                  ) : null}
                </div>
                    ...
                    <div className="mt-2 space-y-1.5">
                      {items.slice(0, 4).map(item => (
                        <button key={item.id} type="button"
                          onClick={() => updateState(current => ({ ...current,
                            kitchenItems: current.kitchenItems.map(entry => entry.id === item.id
                              ? { ...entry, checked: !entry.checked } : entry) }))}
                          className="flex w-full items-center gap-1.5 rounded-xl bg-white/80 px-2 py-1.5 text-left text-[10px]">
            ...
            <section className="mt-4 min-h-0 flex-1 overflow-hidden rounded-[28px] border border-white/90 bg-white/70 p-3 shadow-sm backdrop-blur-xl">
              <div className="flex h-full content-start flex-wrap gap-2 overflow-y-auto overscroll-contain">
                {analysis.ingredients.map(item => {
                  const key = normaliseKitchenItem(item.name);
                  const active = confirmedIngredients.has(key);
                  return (
                    <button key={key} type="button"
                      onClick={() => setConfirmedIngredients(current => {
                        const next = new Set(current);
                        if (next.has(key)) next.delete(key); else next.add(key);
                        return next;
                      })}
                      className={`flex h-9 items-center gap-2 rounded-full border px-3 text-[11px] font-semibold transition ${active ? "border-[#8fa983] bg-[#edf4e9] text-[#55704c]" : "border-slate-200 bg-white text-slate-400 line-through"}`}
                      aria-pressed={active}>
          ...
          <main className="mt-4 flex min-h-0 flex-1 flex-col">
            ...
            <section className="mt-3 grid min-h-0 flex-1 grid-cols-2 gap-2">
              {analysis.mealSuggestions.map((meal, index) => (
                <button key={`${meal.name}-${index}`} type="button"
                  onClick={() => setSelectedMealIndex(index)}
                  className={`min-h-0 overflow-hidden rounded-[22px] border p-3 text-left transition ${selectedMealIndex === index ? "border-[#78936c] bg-[#eef5ea] shadow-[0_12px_26px_-20px_rgba(35,54,43,0.7)]" : "border-white bg-white/72"}`}>
          ...
          <main className="flex min-h-0 flex-1 flex-col items-center justify-center text-center">
            ...
            <p className="mt-2 max-w-xs text-xs leading-5 text-slate-500">
              {selectedMeal.missingIngredients.length
                ? `${selectedMeal.missingIngredients.length} missing item${selectedMeal.missingIngredients.length === 1 ? "" : "s"} added to your shared shopping list.`
                : "You have everything you need for this meal."}
            </p>
