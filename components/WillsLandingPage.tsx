  ...
  return (
    <header className="relative min-h-[390px] overflow-hidden rounded-[32px] border border-[#20352a]/[0.06] bg-[#f5f4ed] px-6 pb-12 pt-[max(1.5rem,env(safe-area-inset-top))] shadow-[0_24px_65px_-45px_rgba(32,53,42,0.34)] sm:min-h-[420px] sm:px-10 sm:pb-14">
      ...
      <div className="relative z-10 flex items-center justify-between">
        <span aria-hidden="true" className="flex h-11 w-11 items-center justify-center text-[#20352a]">
      ...
      <div className="relative z-10 mt-16 max-w-[31rem] sm:mt-20">
        <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-[#6f8e72]">Private planning</p>
...
function WillsMenuCard({ item }: { item: WillsMenuItem }) {
  return (
    <Link
      ...
      className="group flex min-h-[92px] w-full items-center gap-4 rounded-[21px] border border-[#20352a]/[0.08] bg-[#fffdf8] p-4 shadow-[0_18px_40px_-28px_rgba(32,53,42,0.38)] transition-[transform,box-shadow,background-color] duration-200 motion-reduce:transition-none hover:-translate-y-0.5 hover:bg-white hover:shadow-[0_22px_44px_-26px_rgba(32,53,42,0.42)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#6f8e72] focus-visible:ring-offset-2 active:translate-y-0 active:bg-[#f7f8f2] sm:min-h-[98px] sm:p-5"
    >
      ...
      <span className="min-w-0 flex-1">
        <span className="block text-[16px] font-semibold tracking-[-0.01em] text-[#20352a] sm:text-[17px]">{item.title}</span>
        <span className="mt-1 block text-[13px] leading-5 text-[#667068] sm:text-[14px]">{item.description}</span>
      </span>
      <UiIcon name="chevron-right" className="h-5 w-5 shrink-0 text-[#6f786f] transition-transform duration-200 motion-reduce:transition-none group-hover:translate-x-0.5" />
    </Link>
  );
}
...
export function WillsLandingPage() {
  return (
    <div className="mx-auto w-full max-w-[760px] pb-6">
      ...
      <section aria-labelledby="wills-menu-heading" className="mt-5 space-y-3 sm:mt-6 sm:space-y-4">
        <h2 id="wills-menu-heading" className="sr-only">Wills and wishes sections</h2>
        {willsMenuItems.map((item) => <WillsMenuCard key={item.href} item={item} />)}
      </section>
