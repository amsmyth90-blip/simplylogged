...
const backLinkClass = [
  ...
  "border-[#20352a]/10 bg-white/80 text-[#20352a] transition hover:bg-white",
...
const actionLinkClass = [
  "group flex min-h-[78px] items-center gap-3 rounded-[18px] border",
  "border-[#20352a]/[0.07] bg-white px-4 py-3 transition hover:-translate-y-0.5",
  "hover:border-[#6f8e72]/25 hover:shadow-[0_14px_28px_-24px_rgba(32,53,42,0.55)]",
  "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#6f8e72]",
  "focus-visible:ring-offset-2 active:translate-y-0 motion-reduce:transform-none",
...
export function WillSectionHeading({
  icon,
  title,
  description,
}: {
...
export function WillActionLink({
  href,
  icon,
  title,
  detail,
  ...
  return (
    ...
    >
      ...
      <UiIcon
        name="chevron-right"
        className="h-4 w-4 shrink-0 text-[#6f8e72] transition group-hover:translate-x-0.5 motion-reduce:transform-none"
      />
    </Link>
  );
}
...
export function WillLegalNotice() {
  return (
    <p className="rounded-[18px] border border-[#20352a]/[0.07] bg-[#f0f2e9] px-4 py-3.5 text-[12px] leading-5 text-[#667068]">
      DiaryDock helps you organise and securely store your information. It does
      not provide legal advice or replace advice from a qualified solicitor.
    </p>
  );
}
