...
export function LetterVersionsWorkspace({ letterId }: { letterId: string }) {
  ...
  const restore = (versionId: string) => {
    if (!letter) return;
    const selected = letter.versions.find(
      (version) => version.id === versionId,
    );
    if (
      !selected ||
      !window.confirm(
        "Use this wording as a new version? Your existing versions will remain available.",
      )
    )
      return;
    ...
    updateState((current) => {
      ...
      return {
        ...
        willsWishes: {
          ...current.willsWishes,
          lettersOfWishes: {
            letters: currentRecord.letters.map((item) =>
              item.id === letterId
                ? {
                    ...item,
    ...
    setMessage(
      `Version ${selected.versionNumber} was restored as new version ${nextNumber}.`,
    );
  };
  ...
  const versions = [...letter.versions].sort(
          ...
          <li key={version.id}>
            <WillCard as="article">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-[11px] text-[#667068]">
                    Saved {formatWillDate(version.createdAt)}
                  </p>
                </div>
                {index === 0 ? (
              ...
              <details className="mt-4">
                <summary className="flex min-h-11 cursor-pointer items-center justify-between rounded-[13px] bg-[#f3f4ed] px-3 text-sm font-semibold text-[#294436] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#6f8e72]">
                  Read this version{" "}
                  <UiIcon name="chevron-down" className="h-4 w-4" />
                </summary>
                <div className="mt-3 rounded-[14px] border border-[#20352a]/[0.06] bg-white p-4">
                  <h3 className="font-serif text-xl text-[#20352a]">
                    {version.title}
                  </h3>
                  <p className="mt-3 whitespace-pre-wrap font-serif text-[15px] leading-7 text-[#59655d]">
                    {version.content}
                  </p>
                </div>
              </details>
              {index !== 0 ? (
                <button
                  ...
                  className="mt-3 min-h-11 rounded-[14px] border border-[#6f8e72]/30 px-4 text-sm font-semibold text-[#294436]"
                >
