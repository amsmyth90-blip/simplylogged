...
export function MealPlannerShoppingSheet({ planner }: { planner: MealPlannerController }) {
  ...
  return (
    <div className="absolute inset-0 z-[70] flex items-end bg-slate-950/20 p-3 pb-[max(12px,env(safe-area-inset-bottom))] backdrop-blur-[3px]" role="presentation" onClick={() => planner.setShoppingOpen(false)}>
      <section className="mx-auto max-h-[90svh] w-full max-w-lg overflow-y-auto rounded-[28px] border border-white/90 bg-[#fbfcf9]/98 p-4 shadow-2xl" role="dialog" aria-modal="true" aria-label="Weekly shopping list" onClick={(event) => event.stopPropagation()}>
        ...
        <WebGroceryExpiry />
        <div className="mt-4 grid max-h-[30svh] grid-cols-2 gap-2 overflow-y-auto">
          ...
          {!planner.weeklyMissingIngredients.length ? <p className="col-span-2 rounded-2xl bg-[#edf4e9] px-3 py-5 text-center text-xs text-[#607b55]">No missing ingredients found. Check your grocery dates above.</p> : null}
        </div>
