...
import { initials, roleLabel } from "./family-utils";
...
export function HouseholdMembers({ household, busy, onRemove, onRole }: Props) {
  ...
  return (
    <section className="family-card">
      <div className="family-section-title"><h2>App access</h2><span>{household.members.length}</span></div>
        ...
        {household.members.map((member) => {
          ...
          return (
            <article className="family-member" key={member.userId}>
              ...
              <div><strong>{member.name}{isCurrent ? <small> You</small> : null}</strong><span>{roleLabel(member.role)} · {member.relation}</span></div>
