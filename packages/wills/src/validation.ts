...
export function exact(value: JsonRecord, keys: readonly string[], label: string) {
...
export function text(value: unknown, label: string, maximum: number, optional = false) {
  ...
  if ((!optional && !result) || result.length > maximum) throw new Error(`${label} is invalid.`);
  return result;
}
...
export function date(value: unknown, label: string, optional = true) {
  ...
  if (result && !/^\d{4}-\d{2}-\d{2}$/.test(result)) throw new Error(`${label} is invalid.`);
  return result;
}

export function timestamp(value: unknown, label: string, optional = false) {
  const result = text(value, label, 40, optional);
  if (result && !Number.isFinite(Date.parse(result))) throw new Error(`${label} is invalid.`);
  return result;
}
...
export function positiveInteger(value: unknown, label: string, maximum: number) {
