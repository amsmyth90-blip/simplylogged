...
import { Archive, ChevronDown, FileText, LockKeyhole, Plus, Search } from "lucide-react";
                ...
                <article key={drawer} className="overflow-hidden rounded-[1.25rem] bg-white shadow-sm shadow-stone-200">
                  ...
                  {isOpen ? (
                    <div className="space-y-2 border-t border-stone-100 p-3">
                      {drawerDocs.length ? drawerDocs.map((document) => <DocumentCard key={document.id} document={document} />) : <EmptyDrawer />}
                    </div>
                  ) : null}
                </article>
              );
        ...
        <section className="mt-5 rounded-[1.25rem] bg-[#28231d] p-4 text-white shadow-lg shadow-stone-300/40">
          ...
          <div className="mt-3 space-y-2">
            {(recentlyAdded.length ? recentlyAdded : documents.slice(0, 2)).map((document) => (
              <DocumentCard key={`recent-${document.id}`} document={document} dark />
            ))}
            {!documents.length ? (
              <p className="rounded-2xl bg-white/10 p-3 text-sm text-white/70">Add a document to open your first drawer.</p>
            ) : null}
          </div>
        </section>
      </div>
...
function DocumentCard({ document, dark = false }: { document: StoredDocument; dark?: boolean }) {
  return (
    <Link
      href={document.roomId === "vault" ? "/vault" : `/room/${document.roomId}`}
      className={`flex min-h-16 items-center gap-3 rounded-2xl p-3 ${dark ? "bg-white/10 text-white" : "bg-[#fbf7ef] text-[#261c14]"}`}
    >
