...
import { Archive, CheckCircle2, Mail, MoreHorizontal, MoveRight, Trash2 } from "lucide-react";
    ...
    <main className="min-h-svh bg-[#f5efe6] pb-28 text-[#261c14]">
      <section className="relative min-h-[21rem] overflow-hidden rounded-b-[2rem] bg-[radial-gradient(circle_at_68%_42%,rgba(248,113,113,0.14),transparent_18%),linear-gradient(135deg,#182015,#42522d_48%,#1b130d)] px-5 pb-20 pt-5 text-white shadow-2xl shadow-stone-400/40">
        ...
        <div className="relative z-10 flex items-start justify-between">
          ...
          <button className="grid h-11 w-11 place-items-center rounded-full bg-black/24 backdrop-blur-md" aria-label="More">
            ...
            documents.map((document) => (
              <article key={document.id} className="rounded-[1.35rem] bg-white p-4 shadow-sm shadow-stone-200">
                ...
                <div className="mt-4 grid grid-cols-[1fr_1fr_44px] gap-2">
                  <button onClick={() => acceptSuggestion(document)} className="min-h-11 rounded-xl bg-emerald-700 text-sm font-bold text-white">
                    Accept
                  </button>
                  <label className="flex min-h-11 items-center justify-center rounded-xl border border-stone-200 bg-[#fbf7ef] px-2 text-sm font-bold">
                    ...
                    <select value={document.roomId} onChange={(event) => moveRoom(document, event.target.value)} className="min-w-0 bg-transparent outline-none">
                  ...
                  <button onClick={() => remove(document.id)} className="grid min-h-11 place-items-center rounded-xl border border-stone-200 bg-[#fbf7ef]" aria-label="Delete">
