create table if not exists public.app_state ( id text primary key, payload jsonb not null,
...
values ( 'diarydock-documents', 'diarydock-documents', false, 4194304,
...
for select
to authenticated
using ( bucket_id = 'diarydock-documents' and (storage.foldername(name))[1] = (select auth.uid()::text)
);
...
for insert
to authenticated
with check ( bucket_id = 'diarydock-documents' and (storage.foldername(name))[1] = (select auth.uid()::text)
);
...
create table if not exists public.documents ( id text primary key, user_id uuid not null default auth.uid(),
  title text not null, category text not null, kind text not null, size_label text not null, room_id text,
  room_name text, issuer text, due_date text, storage_bucket text, storage_path text, original_file_name text,
  mime_type text, extraction_summary text, extracted_text text, action_items jsonb not null default '[]'::jsonb,
  confidence numeric, review_status text not null default 'reviewed', review_reasons jsonb not null default '[]'::jsonb,
  reviewed_at text, emergency_visible boolean not null default false, shared_with jsonb not null default '[]'::jsonb,
...
create table if not exists public.reminders ( id text primary key, user_id uuid not null default auth.uid(),
  title text not null, note text, room_id text, room_name text, reminder_group text not null, time_label text not null,
  priority text not null, repeat text, document_id text references public.documents(id) on delete set null,
  document_title text, created_at timestamptz not null default timezone('utc', now()),
  updated_at timestamptz not null default timezone('utc', now())
);
...
create table if not exists public.household_members ( id text primary key, user_id uuid not null default auth.uid(),
  name text not null, role text not null, access text not null, access_tone text not null default 'limited',
  note text not null default '', initials text not null default '', manages jsonb not null default '[]'::jsonb,
  last_active text not null default 'Now', created_at timestamptz not null default timezone('utc', now()),
  updated_at timestamptz not null default timezone('utc', now())
);
...
create table if not exists public.family_invites ( id text primary key, user_id uuid not null default auth.uid(),
  name text not null, relation text not null, access text not null, sent_ago text not null default 'Just now',
  initials text not null default '', status text not null default 'pending',
  created_at timestamptz not null default timezone('utc', now()),
  updated_at timestamptz not null default timezone('utc', now())
);
...
create table if not exists public.document_permissions (
  document_id text not null references public.documents(id) on delete cascade, subject_name text not null,
  owner_id uuid not null default auth.uid(), access_level text not null default 'view',
  created_at timestamptz not null default timezone('utc', now()),
  updated_at timestamptz not null default timezone('utc', now()), primary key (document_id, subject_name)
);
...
create table if not exists public.rate_limit_buckets ( key_hash text primary key,
  request_count integer not null default 0 check (request_count >= 0), reset_at timestamptz not null,
  created_at timestamptz not null default timezone('utc', now()),
  updated_at timestamptz not null default timezone('utc', now())
);
...
create or replace function public.check_rate_limit( bucket_key text, max_requests integer, window_seconds integer
)
...
as $$
declare v_now timestamptz := now(); effective_max integer; effective_window_seconds integer; hashed_key text;
  current_count integer; current_reset_at timestamptz;
begin if bucket_key is null or length(bucket_key) < 16 then raise exception 'bucket_key must be at least 16 characters';
  end if; effective_max := greatest(1, least(coalesce(max_requests, 1), 1000));
  effective_window_seconds := greatest(1, least(coalesce(window_seconds, 60), 86400));
  hashed_key := encode(extensions.digest(bucket_key, 'sha256'), 'hex'); insert into public.rate_limit_buckets (
    key_hash, request_count, reset_at, created_at, updated_at ) values ( hashed_key, 1,
    v_now + make_interval(secs => effective_window_seconds), v_now, current_time ) on conflict (key_hash) do update set
    request_count = case when public.rate_limit_buckets.reset_at <= v_now then 1
      else public.rate_limit_buckets.request_count + 1 end, reset_at = case
      when public.rate_limit_buckets.reset_at <= v_now then v_now + make_interval(secs => effective_window_seconds)
      else public.rate_limit_buckets.reset_at end, updated_at = v_now
  returning request_count, public.rate_limit_buckets.reset_at into current_count, current_reset_at;
  delete from public.rate_limit_buckets where public.rate_limit_buckets.reset_at < v_now - interval '1 hour'
    and public.rate_limit_buckets.updated_at < v_now - interval '1 hour'; allowed := current_count <= effective_max;
  remaining := greatest(0, effective_max - current_count); retry_after_seconds := case when allowed then 0
    else greatest(1, ceil(extract(epoch from current_reset_at - v_now))::integer) end; reset_at := current_reset_at;
  return next;
end;
$$;
