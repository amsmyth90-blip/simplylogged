create or replace function public.apply_mobile_kitchen_calendar_state(
  input_user_id uuid,
...
begin
  ...
  if input_user_id is null or input_payload is null
    ...
    raise exception 'Invalid Kitchen calendar state';
  end if;
...
revoke all on function public.apply_mobile_kitchen_calendar_state(
  uuid, timestamptz, jsonb
) from public, anon, authenticated;
grant execute on function public.apply_mobile_kitchen_calendar_state(
  uuid, timestamptz, jsonb
) to service_role;
