-- Save one revision-checked travel change through the server-only boundary.
create or replace function public.apply_mobile_travel_state(
  input_user_id uuid,
...
revoke all on function public.apply_mobile_travel_state(uuid, timestamptz, jsonb)
from public, anon, authenticated;
grant execute on function public.apply_mobile_travel_state(uuid, timestamptz, jsonb)
to service_role;
