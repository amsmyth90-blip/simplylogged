...
create index if not exists resource_permissions_subject_idx
  on public.resource_permissions (subject_user_id, shared_resource_id)
...
declare
  ...
  resource_record public.shared_resources%rowtype;
begin
  ...
  if requested_action not in ('VIEW', 'EDIT') then
    return false;
  end if;

  if target_resource_type = 'vault_document' then
    return false;
  end if;
  ...
  where resource.owner_id = target_owner_id
    and resource.resource_type = target_resource_type
    and resource.resource_id = target_resource_id
  limit 1;

  if resource_record.id is null or resource_record.visibility = 'PRIVATE' then
    return false;
  end if;
  ...
  if not exists (
    ...
    where membership.household_id = resource_record.household_id
  ...
  if not exists (
    ...
    where owner_membership.household_id = resource_record.household_id
      and owner_membership.user_id = target_owner_id
      and owner_membership.status = 'active'
  ) then
    return false;
  end if;

  if resource_record.visibility = 'HOUSEHOLD' then
    return requested_action = 'VIEW';
  end if;

  return exists (
    select 1
    from public.resource_permissions as permission
    where permission.shared_resource_id = resource_record.id
      and permission.subject_user_id = current_user_id
      and permission.revoked_at is null
      and case
        when requested_action = 'EDIT' then permission.can_edit
...
begin
  ...
  select
    resource.visibility,
    coalesce(
      array_agg(permission.subject_user_id)
        filter (where permission.subject_user_id is not null and permission.revoked_at is null),
      '{}'::uuid[]
    )
  from public.shared_resources as resource
  left join public.resource_permissions as permission
    on permission.shared_resource_id = resource.id
  where resource.owner_id = auth.uid()
    and resource.resource_type = 'document'
    and resource.resource_id = target_document_id
  group by resource.visibility;

  if not found then
    return query select 'PRIVATE'::text, '{}'::uuid[];
  end if;
end;
$$;
...
as $$
begin
  delete from public.shared_resources
...
using (
  owner_id = auth.uid()
  or public.can_access_shared_resource(resource_type, resource_id, owner_id, 'VIEW')
);
...
using (
  ...
  or exists (
    select 1
    from public.shared_resources as resource
    where resource.id = shared_resource_id
