-- Preserve native ID types across legacy text and deployed UUID schemas.
begin;
...
declare
  ...
  document_key public.documents.id%type;
  reminder_key public.reminders.id%type;
  reminder_document_key public.reminders.document_id%type;
...
begin
  ...
  document_key := reservation.document_id;
  reminder_document_key := reservation.document_id;
  if reminder is not null then reminder_key := reminder ->> 'id'; end if;
  if reservation.committed_at is not null then
    return exists (
      select 1 from public.documents where id = document_key
  ...
  insert into public.documents (
    ...
    document_key, input_user_id, trim(input_metadata ->> 'title'),
  ...
  if reminder is not null then
    insert into public.reminders (
      ...
      reminder_key, input_user_id, trim(reminder ->> 'title'),
      ...
      reminder_document_key, trim(input_metadata ->> 'title')
    ) on conflict (id) do nothing;
    if not exists (
      select 1 from public.reminders where id = reminder_key
        and user_id = input_user_id and document_id = reminder_document_key
    ) then return false; end if;
  end if;
