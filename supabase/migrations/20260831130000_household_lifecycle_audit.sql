...
begin
  if old.status = 'active' and new.status = 'removed' then
    update public.resource_permissions as permission
    set revoked_at = timezone('utc', now())
    from public.shared_resources as resource
    where permission.shared_resource_id = resource.id
      and resource.household_id = new.household_id
      and permission.subject_user_id = new.user_id
...
begin
  if tg_op = 'INSERT' and new.status = 'active' then
    audit_event_type := 'HOUSEHOLD_JOIN';
    audit_metadata := jsonb_build_object(
      'subjectUserId', new.user_id::text,
      'role', new.role
    );
  elsif tg_op = 'UPDATE' and old.status = 'active' and new.status = 'removed' then
    audit_event_type := case
      when actor_user_id = new.user_id then 'HOUSEHOLD_LEFT'
      else 'HOUSEHOLD_MEMBER_REMOVED'
    end;
    audit_metadata := jsonb_build_object('subjectUserId', new.user_id::text);
  elsif tg_op = 'UPDATE' and old.role is distinct from new.role then
    audit_event_type := 'HOUSEHOLD_ROLE_CHANGED';
    audit_metadata := jsonb_build_object(
      'subjectUserId', new.user_id::text,
...
begin
  ...
  if tg_op = 'INSERT' then
    audit_event_type := 'HOUSEHOLD_INVITE';
    ...
    audit_event_type := 'HOUSEHOLD_INVITE_RENEWED';
  else
    return new;
  end if;
  ...
  values (
    ...
    actor_user_id::text,
    audit_event_type,
    jsonb_build_object('invitedRole', new.role)
  );
...
begin
  ...
  if invite_role not in ('member', 'viewer') then
    raise exception 'Choose Adult or Member access';
  end if;
  ...
  if clean_email = lower(coalesce(auth.jwt() ->> 'email', '')) then
    raise exception 'Invite another person rather than your own account';
  end if;
  ...
  values (
    ...
    clean_email,
    clean_name,
    coalesce(nullif(trim(invite_relation), ''), 'Household member'),
    case
      when invite_role = 'member' then 'Adult - can contribute to shared household spaces'
      else 'Member - can view items deliberately shared with them'
    end,
    invite_role,
    auth.uid()
  )
...
begin
  ...
  if public.household_role(current_household_id) <> 'owner' then
    raise exception 'Only the household owner can rename the household';
  end if;
...
as $$
declare
  current_membership public.household_memberships%rowtype;
...
begin
  ...
  if current_membership.household_id is null then
    raise exception 'This account does not have an active household';
  end if;

  if current_membership.role = 'owner' then
    raise exception 'The household owner cannot leave until ownership transfer is available';
  end if;
  ...
  values (
    ...
    auth.uid(),
    'owner',
    current_membership.display_name,
    'Household owner'
  );
