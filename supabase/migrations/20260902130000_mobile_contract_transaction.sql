-- Save one revision-checked Office contract and linked document metadata atomically.
create or replace function public.apply_mobile_contract_state(
...
begin
  ...
  if input_expected_revision is null then
    ...
    update public.app_state set payload = input_payload
...
from public, anon;
grant execute on function public.apply_mobile_contract_state(timestamptz, jsonb, jsonb)
to authenticated;
