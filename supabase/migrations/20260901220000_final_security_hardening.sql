...
create function public.set_document_sharing(
...
begin
  perform public.require_recent_authentication(900);
  return query
  select result.shared_resource_id, result.visibility
  from public.set_document_sharing_without_recent_auth(
    target_document_id,
    new_visibility,
    selected_user_ids
  ) as result;
...
alter table public.documents
  add constraint documents_storage_binding_check
  check (
...
as $$
  select exists (
    ...
    where document.storage_bucket = 'diarydock-documents'
      and document.storage_path = object_name
      and split_part(document.storage_path, '/', 1) = document.user_id::text
      and split_part(document.storage_path, '/', 2) = document.id::text
      and (
...
begin
  ...
  if action_record.id is null then
    raise exception 'Completed action not found or access denied';
  end if;
