-- Route legacy noticeboard mutations through a server-only, revision-checked boundary.
create or replace function public.apply_mobile_kitchen_notice_state(
  input_user_id uuid,
...
begin
  ...
  if input_delete_reminder_id is not null then
    ...
    where id = input_delete_reminder_id and user_id = input_user_id;
  end if;
  ...
  if input_reminder is not null then
    ...
    insert into public.reminders (
      ...
      reminder_id, input_user_id, trim(input_reminder ->> 'title'),
    ...
    where public.reminders.user_id = input_user_id;
  end if;
...
revoke all on function public.apply_mobile_kitchen_notice_state(
  uuid, timestamptz, jsonb, jsonb, text
) from public, anon, authenticated;
grant execute on function public.apply_mobile_kitchen_notice_state(
  uuid, timestamptz, jsonb, jsonb, text
) to service_role;
