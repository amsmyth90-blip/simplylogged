-- Save one revision-checked pantry or shopping change through the server-only boundary.
create or replace function public.apply_mobile_kitchen_items_state(
  input_user_id uuid,
...
revoke all on function public.apply_mobile_kitchen_items_state(uuid, timestamptz, jsonb)
from public, anon, authenticated;
grant execute on function public.apply_mobile_kitchen_items_state(uuid, timestamptz, jsonb)
to service_role;
