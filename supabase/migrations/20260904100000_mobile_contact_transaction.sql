-- Save one revision-checked Office contact change through the server-only boundary.
create or replace function public.apply_mobile_contact_state(
  input_user_id uuid,
...
begin
  ...
  if input_expected_revision is null then
    ...
    where id = input_user_id::text and updated_at = input_expected_revision
    returning * into state_row;
  end if;
...
revoke all on function public.apply_mobile_contact_state(uuid, timestamptz, jsonb)
from public, anon, authenticated;
grant execute on function public.apply_mobile_contact_state(uuid, timestamptz, jsonb)
to service_role;
