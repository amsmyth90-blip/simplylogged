...
The current Capacitor apps contain a signed local mobile bundle (`capacitor.config.ts`, `apps/mobile/package.json`) distributed independently through the application stores. That establishes the required code-distribution boundary, but it does not itself provide Vault E2EE: the present document APIs, metadata, OCR and stored files remain server-readable. The release gate for the strong claim “DiaryDock's server cannot decrypt” is a reviewed cryptographic implementation inside that signed bundle, platform secure-key storage, recovery and multi-device flows, migration evidence and adversarial interoperability tests. The web app may be read-only or unavailable for strong-mode Vault content unless an approved WebAuthn-based boundary meets the same threat model.
