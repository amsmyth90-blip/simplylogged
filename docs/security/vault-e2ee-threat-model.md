...
| Document upload | File bytes | Private bucket, owner path, 10 MB/type allow-list | `diarydock-documents/<user>/<document>/<name>` | User browser, Supabase Storage, authorised signed-link readers | Storage RLS and bucket constraints | `lib/document-storage.ts:17-40`, `supabase/schema.sql:37-91` |
