import type { SearchCandidate, SearchResult } from "@/lib/search/results";

export type AskCitation = SearchResult & { ref: string };

const stopWords = new Set([
  "a", "about", "all", "an", "and", "are", "before", "did", "do", "does", "for", "from", "have", "i", "in", "is", "it", "me", "my", "of", "on", "or", "show", "still", "the", "this", "to", "was", "what", "when", "where", "which", "with", "year"
]);

function words(value: string) {
  return value.toLocaleLowerCase("en-GB").replace(/[^a-z0-9]+/g, " ").trim().split(/\s+/).filter((word) => word.length > 1 && !stopWords.has(word));
}
...
export function retrieveAskCitations(candidates: SearchCandidate[], question: string, now = new Date(), limit = 8): AskCitation[] {
  const normalized = question.toLocaleLowerCase("en-GB");
  ...
  const ranked = candidates.map((candidate) => {
    ...
    for (const word of queryWords) {
      if (candidate.title.toLocaleLowerCase("en-GB").includes(word)) score += 8;
      else if (haystack.includes(word)) score += 3;
    }
    if ([...domains].some((domain) => candidate.domains.includes(domain as SearchCandidate["domains"][number]))) score += 5;
    if (dueIntent && candidate.dueAt) score += 6;
    if (horizon && candidate.dueAt) {
      const due = new Date(candidate.dueAt);
      if (due >= now && due <= horizon) score += 14;
      else score -= 8;
    }
    if (normalized.includes("not reviewed") && candidate.badge === "Review") score += 12;
    return { candidate, score };
  }).filter(({ candidate, score }) => score > 0 && (!horizon || (Boolean(candidate.dueAt) && new Date(candidate.dueAt as string) >= now && new Date(candidate.dueAt as string) <= horizon)))
    .sort((left, right) => right.score - left.score || Date.parse(left.candidate.dueAt || "9999-12-31") - Date.parse(right.candidate.dueAt || "9999-12-31") || left.candidate.title.localeCompare(right.candidate.title))
