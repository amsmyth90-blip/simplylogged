export const searchCategories = ["all", "documents", "reminders", "home", "vehicles", "pets", "travel", "insurance", "contacts", "assets"] as const;
...
export type SearchCandidate = {
  ...
  domains: Exclude<SearchCategory, "all">[];
  title: string;
  detail: string;
  href: string;
  dueAt?: string;
...
export type SearchResult = Omit<SearchCandidate, "searchText" | "updatedAt" | "domains">;
...
function score(candidate: SearchCandidate, query: string) {
  if (!query) return candidate.updatedAt ? Math.max(0, Date.parse(candidate.updatedAt) / 1e12) : 0;
...
export function filterAndRankSearchResults(candidates: SearchCandidate[], query: string, category: SearchCategory = "all", dateFilter: SearchDateFilter = "all", now = new Date()) {
  const normalized = query.trim().toLocaleLowerCase("en-GB").replace(/\s+/g, " ");
  return candidates
    .map((candidate) => ({ candidate, rank: score(candidate, normalized) }))
    .filter(({ candidate, rank }) => (!normalized || rank > 0) && (category === "all" || candidate.domains.includes(category)))
    .filter(({ candidate }) => {
    ...
    .sort((left, right) => right.rank - left.rank || Date.parse(left.candidate.dueAt || "9999-12-31") - Date.parse(right.candidate.dueAt || "9999-12-31") || left.candidate.title.localeCompare(right.candidate.title))
    .map(({ candidate }) => ({
      id: candidate.id,
