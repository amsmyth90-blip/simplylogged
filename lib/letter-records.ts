...
export const letterRecipientOptions: Array<{ value: LetterRecipientType; label: string; description: string }> = [
...
export const letterPurposeOptions: Array<{ value: LetterPurpose; label: string; description: string }> = [
...
export function hydrateLettersRecord(record: Partial<LettersOfWishesRecord> | null | undefined): LettersOfWishesRecord {
  const initialLetter = createLetterDraft();
  return {
    letters: Array.isArray(record?.letters)
      ? record.letters.map((letter) => ({
          ...initialLetter,
          ...letter,
          attachmentDocumentIds: Array.isArray(letter.attachmentDocumentIds) ? letter.attachmentDocumentIds : [],
          delivery: { ...initialLetter.delivery, ...(letter.delivery ?? {}) },
          deliveryActivation: "not-active",
