...
const BrandBug: React.FC = () => (
  <div style={{position: 'absolute', top: 84, left: 72, display: 'flex', alignItems: 'center', gap: 16}}>
    ...
    <div style={{color: 'white', fontSize: 30, fontWeight: 850, letterSpacing: 2.4, textShadow: '0 3px 14px rgba(0,0,0,.35)'}}>DIARYDOCK</div>
  </div>
);
  ...
  return (
    <AbsoluteFill style={{background: 'linear-gradient(150deg,#edf6ff,#f5f0ff)', padding: '110px 70px 90px'}}>
      <div style={{display: 'flex', alignItems: 'center', gap: 18}}>
        <Img src={staticFile('brand/diarydock-mark.png')} style={{width: 86, height: 86, objectFit: 'contain'}} />
        <div style={{fontSize: 42, color: navy, fontWeight: 900, letterSpacing: 3}}>DIARYDOCK</div>
      </div>
      ...
      <div style={{marginTop: 74, height: 112, borderRadius: 30, background: 'white', boxShadow: '0 22px 62px rgba(33,73,130,.16)', display: 'flex', alignItems: 'center', padding: '0 34px', gap: 24}}>
        ...
        <div style={{fontSize: 40, color: typed ? navy : '#8ca0b8', fontWeight: 720}}>{typed || 'Search DiaryDock'}</div>
...
const EndCard: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{background: '#f3f6fc', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: 74, opacity}}>
      ...
      <div style={{fontSize: 110, fontWeight: 900, letterSpacing: 8, marginTop: 28, background: 'linear-gradient(90deg,#06336d,#477dd1,#8a62e8)', WebkitBackgroundClip: 'text', color: 'transparent'}}>DIARYDOCK</div>
